...
class HidroPixel:
    ...
    def escreve_RDC(self, nome_RST, varmin, varmax):
        """
        ...
        with open(nome_rdc, 'w') as rdc_file:
            ...
            self.global_vars.varaux = varmin
            ...
            self.global_vars.varaux = varmax
            self.rdc_vars.num = 2 # num = 2 : real
            textoaux = 'max. value  : '
            tamnum = self.tamnum(self.global_vars.varaux, self.rdc_vars.num)
            self.aux_RDC(rdc_file, testoaux,self.global_vars.varaux, tamnum)
    ...
    def escreve_comprimento_acumulado(self, name, matriz, tipodado, qtd=1, *args):
        """
        Esta função é responsável por formular os arquivos de saída (tanto o raster (.rst), quanto sua documentação (.rdc))
        para os da bacia hidrográfica

        Parametros:

        name = nome que o arquivo de saída receberá
        matriz = array que contém as infomações processadas do arquivo que será escrito
        tipodado = corresponde ao tipo dos elemntos do array: 1 = integer, 2 = float
        qtd = infica quantos arquivos serão inscritos com a execução da função
        *args = parametros ligados ao segundo arquivo: name, array e tipo de dado (na ordem mostrada)
        ...
        with open(fn, 'wb') as arquivo1:
            # Escrevendo o resultado do comprimento da rede de drenagem
            for lin in range(self.rdc_vars.nlin):
                dados_fn = [float(matriz[lin][col]) for col in range(self.rdc_vars.ncol)]
                arquivo1.write(dados_fn.tobytes())
        ...
        self.rdc_vars.tipo_dado = tipodado

        # JVDvars: variáveis em desuso (tipoMM & i3)
        ...
        # JVDfunc: substitui a rotina MinMax
        varmin = np.min(matriz)
        varmax = np.max(matriz)
        self.escreve_RDC(nomeRST, varmin, varmax)

        # JVDfunc: condição para criar mais de um arquivo
        if qtd == 2 or qtd = 3:
            # Escrevendo o resultado do comprimento da rede de drenagem
            fn2 = args[0]
            with open(fn2, 'wb') as arquivo2:
                for lin in range(self.rdc_vars.nlin):
                    dados_fn2 = [float(args[1][lin][col]) for col in range(self.rdc_vars.ncol)]
                    arquivo2.write(dadosfn2.tobytes())
            # Alocando as variáveis para escrita da documentação do arquivo rdc para o comprimento da foz da bacia hidrográfica
            self.rdc_vars.tipo_dado = args[2]
            nomeRST2 = fn2
            varmax2 = np.max(args[1])
            varmin2 = np.min(args[1])
            self.escreve_RDC(nomeRST2, varmin2, varmax2)
            
            if qtd == 3:
                # Escrevendo o resultado do comprimento da rede de drenagem
                fn3 = args[3]
                with open(fn3, 'wb') as arquivo3:
                    for lin in range(self.rdc_vars.nlin):
                        dados_fn3 = [float(args[4][lin][col]) for col in range(self.rdc_vars.ncol)]
                        arquivo3.write(dadosfn3.tobytes())
                # Alocando as variáveis para escrita da documentação do arquivo rdc para o comprimento da foz da bacia hidrográfica
                self.rdc_vars.tipo_dado = args[5]
                nomeRST3 = fn3
                varmax3 = np.max(args[4])
                varmin3 = np.min(args[4])
                self.escreve_RDC(nomeRST3, varmin3, varmax3)
        ...
        with open('dados_trechos_superf.txt') as arquivo_txt:
            ...
            for t in range(self.global_vars.numtreaux):
                if self.global_vars.usotre[self.global_vars.numcabeaux][t] == 0:
                        ...
                        input('Press enter to continue...') # Alterar para o qgis!
                        
                # Escrevendo as linhas do arquivo conforme os valores das variáveis
                arquivo_txt.write('{:<10}{:<6}{:<10.2f}{:<10.2f}{:<10.2f}{:<12.6f}{:<6}\n'.format(self.global_vars.numcabeaux, t, self.global_vars.Ltre[self.global_vars.numcabeaux][t], \
                    self.global_vars.cotaini[self.global_vars.numcabeaux][t], self.global_vars.cotafim[self.global_vars.numcabeaux][t],\
